...
class Jwt_Auth_Public
{
    ...
    public function add_api_routes()
    {
        ...
        register_rest_route($this->namespace, 'token/validate_userid', array(
            'methods' => 'POST',
            'callback' => array($this, 'validate_token_userid'),
        ));
    ...
    public function generate_jwt($param){
        $token = JWT::encode(apply_filters('jwt_auth_token_before_sign', $param->token, $param->user), $param->secret_key);
        $data = array(
            'token' => $token,
            'param' => $param,
    ...
    //add_filter('jwt_generate_filter', 'generate_jwt');
    ...
    public function validate_token_userid($output = true)
    {
        /*
         * Looking for the HTTP_AUTHORIZATION header, if not present just
         * return the user.
        ...
        try {
            ...
            $user_data = get_userdata($token->data->user->id);
            /** If the output is true return an answer to the request to show it */
             return array(
                 'code' => 'jwt_auth_valid_token',
                 'data' => array(
